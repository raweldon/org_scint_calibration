...
def spectra_fit(fit_params, *args, **kwargs):
    ...
    return shift_term, spread_term, res
        ...
        for idx, g in enumerate(group):
            ...
            if 'co_' in g:
                ...
                if idx == 1:
                    ...
                    shift, spread, res = spectra_fit(fit_params, cs_data, na_data, co_data, print_info=print_info, show_plots=show_plots)
            ...
            if co_cal == False:
                shift, spread, res = spectra_fit(fit_params, cs_data, na_data, print_info=print_info, show_plots=show_plots)
...
def single(adc_ranges, det_no, names, database_dir, sim_files, sim_numpy_files, run, exp_cal_term, spread, beam_4_MeV, print_info, show_plots):
    ...
    for name in names:
        ...
        shift, spread, res = spectra_fit(fit_params, data, print_info=print_info, show_plots=show_plots)
        plt.show()
...
if __name__ == '__main__':
    ...
    for exp_cal_term, det_no, name in zip(exp_cal_terms, det_nos, names):
        ...
        simultaneous(adc_ranges, det_no, name, database_dir, sim_files, sim_numpy_files, run, exp_cal_term, spread, beam_4mev, 
                     save_arrays=False, print_info=True, show_plots=True)
        plt.show()
