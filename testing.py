...
def assign_fit_params(spread_val):
    ...
    fit_params.add('beta_1', value=0.03, min=0, max=0.07, vary=True)
    fit_params.add('gamma_1', value=0.0, min=0, max=20, vary=False)
    fit_params.add('alpha_2', value=0.0, min=0., max=20., vary=False)
    fit_params.add('beta_2', value=0.03, min=0, max=0.07, vary=True)
...
def simultaneous(adc_ranges, det_no, names, database_dir, sim_files, sim_numpy_files, exp_cal_term, spread, intercept,
    ...
    for name in names:
        ...
        for idx, run in enumerate(runs):
            ...
            if 'cs_' in run:
                ...
                sim_data_cs[0] = sim_data_cs[0] - intercept
            ...
            if 'na_' in run:
                ...
                sim_data_na[0] = sim_data_na[0] - intercept
            ...
            if 'co_' in run:
                ...
                sim_data_co[0] = sim_data_co[0] - intercept
                ...
                if idx == 1:   
                    ...
                    fit_params.add('beta_3', value=0.03, min=0, max=0.07, vary=True)
...
def fit(names, results, database_dir, beam_4mev, bvert):
    ...
    res_array = simultaneous(adc_ranges, det_no, names, database_dir, sim_files, sim_numpy_files, exp_cal_term, spread, intercept, beam_4mev, 
                             print_info=True, show_plots=True)
...
def save_to_df(results):
    ...
    for r in results:
        for res in r:
            ...
            m, m_std, b, b_std = unpack_res(res)
            data.append([m, m_std, b, b_std, res])
            #data.append([m, m_std, b, b_std, correl_mb, res])

    df = pd.DataFrame(data, index=[energy, crystal], columns=['m', 'm_std', 'b', 'b_std', 'res'])
...
def calculate_calibration(df, database_dir_4, database_dir_11, bvert_11, cpvert_11, bvert_4, cpvert_4, save_arrays):
    ...
    if bvert_11:
        ...
        print '11MeV  bvert   {:^8.2f} {:>8.2f} {:>8.2f} {:>8.2f}'.format(m_11_bvert, np.std(df_11_bvert['m'].values), b_11_bvert, 
                                                                        np.std(df_11_bvert['b'].values))
    ...
    if cpvert_11:
        ...
        print '11MeV cpvert   {:^8.2f} {:>8.2f} {:>8.2f} {:>8.2f}'.format(m_11_cpvert, np.std(df_11_cpvert['m'].values), b_11_cpvert, 
                                                                        np.std(df_11_cpvert['b'].values))
    ...
    if bvert_4:
        ...
        print ' 4MeV  bvert   {:^8.2f} {:>8.2f} {:>8.2f} {:>8.2f}'.format(m_4_bvert, np.std(df_4_bvert['m'].values), b_4_bvert, 
                                                                      np.std(df_4_bvert['b'].values))
    ...
    if cpvert_4:
        ...
        print ' 4MeV cpvert   {:^8.2f} {:>8.2f} {:>8.2f} {:>8.2f}'.format(m_4_cpvert, np.std(df_4_cpvert['m'].values), b_4_cpvert, 
                                                                       np.std(df_4_cpvert['b'].values))
